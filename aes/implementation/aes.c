#include "aes.h"
#include <string.h>
#include <stdlib.h>

// ============================================================
// AES S-box（FIPS 197 标准值，硬编码为全局常量）
// 通过 aes.h 中的 extern 声明在其他文件中使用
// ============================================================
const uint8_t sbox[256] = {
    
};

// ============================================================
// GF(2^8) 基础运算（模多项式 0x11b）
// ============================================================

// 俄罗斯农夫乘法：在 GF(2^8) 中计算 a*b
uint8_t gf_mult(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;           // 若 b 最低位为1，则加上 a
        uint8_t hi_bit = a & 0x80;
        a <<= 1;
        if (hi_bit) a ^= 0x1b;       // 模多项式 x^8 + x^4 + x^3 + x + 1
        b >>= 1;
    }
    return p;
}

// 快速幂：在 GF(2^8) 中计算 a^b
static uint8_t gf_pow(uint8_t a, int b) {
    uint8_t result = 1;
    while (b > 0) {
        if (b & 1) result = gf_mult(result, a);
        a = gf_mult(a, a);
        b >>= 1;
    }
    return result;
}

// 乘法逆元：a^254（群阶为255，a^255 = 1，故 a^-1 = a^254）
uint8_t gf_inverse(uint8_t a) {
    if (a == 0) return 0; // 0 无逆元，AES 规定映射为 0
    return gf_pow(a, 254);
}

// ============================================================
// AES 字节替换（SubBytes）
// ============================================================
uint8_t *sub_bytes(uint8_t *state) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
    return state;
}
uint8_t *shift_rows(uint8_t *state) {
    uint8_t temp;
    // AES state 是列优先存储：
    //   行0: state[0]  state[4]  state[8]  state[12]
    //   行1: state[1]  state[5]  state[9]  state[13]
    //   行2: state[2]  state[6]  state[10] state[14]
    //   行3: state[3]  state[7]  state[11] state[15]

    // Row 0 不变
    // Row 1 左移1位
    temp = state[1];
    state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    // Row 2 左移2位
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    // Row 3 左移3位（等价于右移1位）
    temp = state[15];
    state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = temp;
    return state;
}
uint8_t *mix_columns(uint8_t *state) {
    uint8_t temp[16];
    for (int c = 0; c < 4; c++) {
        int idx = c * 4;
        temp[idx]   = gf_mult(0x02, state[idx]) ^ gf_mult(0x03, state[idx + 1]) ^ state[idx + 2] ^ state[idx + 3];
        temp[idx+1] = state[idx] ^ gf_mult(0x02, state[idx + 1]) ^ gf_mult(0x03, state[idx + 2]) ^ state[idx + 3];
        temp[idx+2] = state[idx] ^ state[idx + 1] ^ gf_mult(0x02, state[idx + 2]) ^ gf_mult(0x03, state[idx + 3]);
        temp[idx+3] = gf_mult(0x03, state[idx]) ^ state[idx + 1] ^ state[idx + 2] ^ gf_mult(0x02, state[idx + 3]);
    }
    memcpy(state, temp, 16);
    return state;
}
uint8_t *add_round_key(uint8_t *state, uint8_t *roundKey) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
    return state;
}
uint8_t* encrypt(uint8_t* plaintext, uint8_t* roundKeys, int rounds) {
    uint8_t state[16];
    memcpy(state, plaintext, 16);
    
    add_round_key(state, roundKeys); // 初始轮密钥加
    for (int round = 1; round <= rounds - 1; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, roundKeys + round * 16);
    }
    // 最后一轮（不含 MixColumns）
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, roundKeys + rounds * 16);
    
    uint8_t* ciphertext = malloc(16);
    memcpy(ciphertext, state, 16);
    return ciphertext;
}
uint8_t rcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

void rot_word(uint8_t *word) {
    uint8_t temp = word[0];
    word[0] = word[1]; word[1] = word[2]; word[2] = word[3]; word[3] = temp;
}

void sub_word(uint8_t *word) {
    for (int i = 0; i < 4; i++) word[i] = sbox[word[i]];
}

void key_expansion(const uint8_t *key, uint8_t *roundKeys, int nk) {
    int nb = 4;
    int nr = nk + 6;
    int total_words = nb * (nr + 1);
    memcpy(roundKeys, key, nk * 4);

    for (int i = nk; i < total_words; i++) {
        uint8_t temp[4];
        memcpy(temp, roundKeys + (i - 1) * 4, 4);
        if (i % nk == 0) {
            rot_word(temp);
            sub_word(temp);
            temp[0] ^= rcon[i / nk - 1];
        }
        for (int j = 0; j < 4; j++) {
            roundKeys[i * 4 + j] = roundKeys[(i - nk) * 4 + j] ^ temp[j];
        }
    }
}